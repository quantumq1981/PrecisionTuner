...
class YinProcessor extends AudioWorkletProcessor {
  ...
  private detectPitch(frame: Float32Array, sr: number): { freq: number; confidence: number } | null {
    ...
    const candidates = [1, 2, 3, 4, 0.5, 1 / 3, 0.25]
      .map((m) => Math.round(baseTau * m))
      .filter((tau) => tau >= tauMin && tau <= tauMax);
  ...
  private rmsDb(frame: Float32Array): number {
    let sum = 0;
